...
// WAVES ONE Computer Agent protocol, version 2.
// Single source of truth for capabilities, risk, job kinds, policy shape,
// filesystem roots, domain policy, command classification, and lifecycle.
// The agent mirrors the enforcement tables in agent/src/policy.mjs;
// src/lib/agent-parity.test.ts asserts parity between the two.
//
// Phase 1 guarantees preserved: pairing, device auth, granular capabilities,
// risk classification, server+agent enforcement, single-use approvals, replay
// protection, STOP/cancel/pause, heartbeats, revocation, append-only audit,
// redaction, before/after state, sandbox escape protection.
...
// Deterministic remote lifecycle. QUEUED awaits authorization; AUTHORIZED is
// releasable to the agent; DISPATCHED left the server; RUNNING is acked by
// the agent; VERIFYING is server-side result validation; then terminal.
export type JobStatus =
  | 'queued' | 'authorized' | 'dispatched' | 'running' | 'verifying'
...
export function effectiveCapability(kind: JobKind, commandClass?: CommandClass): CapabilityId {
  if (kind === 'term.exec') {
...
export interface Policy {
  ...
  // Explicitly authorized filesystem roots. Every path job must resolve
  // inside one of these. Empty means "workspace default" (filled by the
  // control plane at load).
  roots: string[];
  domains: DomainPolicy;
}

export const DEFAULT_DOMAINS: DomainPolicy = {
...
export const DEFAULT_POLICY: Policy = {
  ...
  roots: [],
  domains: { ...DEFAULT_DOMAINS, allowed: [...DEFAULT_DOMAINS.allowed], blocked: [] },
};
...
export function migratePolicy(stored: unknown): Policy {
  const next: Policy = structuredClone(DEFAULT_POLICY);
  if (!stored || typeof stored !== 'object') return next;
  const old = (stored as { capabilities?: Record<string, PolicyValue> }).capabilities || {};
  const take = (key: string): PolicyValue | undefined => {
    const value = old[key];
...
export function validatePolicy(policy: Policy): void {
  if (!policy || typeof policy !== 'object' || (policy as { version?: unknown }).version !== 2) {
...
export function isUrlAllowed(rawUrl: string, domains: DomainPolicy): boolean {
  let parsed: URL;
  try {
  ...
  if (scheme === 'data:' || rawUrl.toLowerCase().startsWith('about:blank')) return true;
...
export type CommandClass = 'readonly' | 'gated' | 'admin' | 'denied';
...
export function resolveAcrossRoots(
  ...
  if (pinnedRoot !== undefined) {
    const match = roots.find(r => r.toLowerCase() === String(pinnedRoot).toLowerCase());
...
export function scrubSecretParams(kind: JobKind, params: Record<string, unknown>): Record<string, unknown> {
  if (kind === 'browser.type' && typeof params.text === 'string') {
    // Idempotent: display paths re-scrub stored records. Never re-hash the
    // marker, or the binding proof would be destroyed by reading it.
    if (params.text === '[REDACTED]') return params;
